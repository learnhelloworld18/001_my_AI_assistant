...
Generated by `make_diagram.py` — a script rather than a drawn image, so a wrong
edge shows up as a diff in review rather than sitting there being wrong. The
same thing as text below, which renders without graphviz:
